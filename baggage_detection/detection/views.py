...
class ImageView(TemplateView):
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            print('Running inference for {}... '.format(form.instance.image.name), end='')

            image = Image.open(form.instance.image).convert('RGB')

            # image = cv2.imread(IMAGE_PATHS)
            # image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            image_rgb = np.array(image)
            image = image_rgb[:, :, ::-1].copy()
            image_expanded = np.expand_dims(image_rgb[:, :, ::-1].copy(), axis=0)
            ...
            img_path = f"/media/images/labeled_{form.instance.image.name.split('/')[-1]}"
